...
if(isset($_POST["submit"])){
    ...
    if($password === $cpassword){
        ...
        if($image_size > 524880){
            ...
            $sql = "UPDATE libraian  SET name='$name' , password='$password', image= '$image_new_name' WHERE id=  id = '{$data["id"]}'";
            $result = mysqli_query($con,$sql);
            if($result){
                echo "<script>alert('Profile Updated Sucessfully.');</script>";
                move_uploaded_file($image_tmp_name, "../images/libraian/" . $image_new_name);
            }else{
...
<div class="content-header">
    ...
    <div class="wrapper">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            <?php
            $sql = "SELECT *FROM libraian WHERE id = '{$data["id"]}'";
            $result = mysqli_query($con, $sql);
            ...
            ?>
                    ...
                    <img src="../images/libraian/<?php echo $row['image']?>" width="150px" height="auto" alt=""><hr>
            <?php
