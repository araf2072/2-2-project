            ...
            <a href="issuebooks.php">
                <div class="panel-content">
                    <div class="row">
                        ...
                        <?php
                        $id =  $data['id'];
                        $sql = "SELECT `issue_book`.`isssue_date`,`books`.`book_name`,`books`.`book_image` FROM `books` INNER JOIN `issue_book` ON `issue_book`.`book_id` = `books`.`id` WHERE `issue_book`.`student_id` = $id AND `issue_book`.`return_date` = '' ";
                        $res = mysqli_query($con,$sql);
                        $total_issue_books = mysqli_num_rows($res);
                        ?>
                        <div class="col-xs-8">
